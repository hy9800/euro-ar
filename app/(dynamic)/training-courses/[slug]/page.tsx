...
export async function generateMetadata({
  ...
  try {
    ...
    const description =
      ...
      `Professional ${category.title} training courses at الدول التدريبية العالمية. Expert-led programs designed to enhance your skills and career prospects.`;
    const keywords = `${
      category.title
    }, training courses, professional development, ${category.title.toLowerCase()} certification, الدول التدريبية العالمية`;
    ...
    return {
      title: "Training Courses | الدول التدريبية العالمية",
      description:
        "Professional training courses at الدول التدريبية العالمية. Expert-led programs designed to enhance your skills and career prospects.",
      keywords:
        "training courses, professional development, certification, الدول التدريبية العالمية",
      authors: [{ name: "الدول التدريبية العالمية" }],
      creator: "الدول التدريبية العالمية",
      publisher: "الدول التدريبية العالمية",
      // robots: {
...
export default async function Page({
  ...
  const categorySchema = {
    ...
    description:
      category.description ||
      `Professional ${category.title} training courses at الدول التدريبية العالمية`,
    url: `${baseUrl}/training-courses/${slug}`,
    breadcrumb: {
      "@type": "BreadcrumbList",
      itemListElement: [
  ...
  return (
    <>
      ...
      <Schema
        pageType="courses"
        pageTitle={`${category.title} دورات تدريبية | الدول التدريبية العالمية`}
        pageDescription={
          category.description ||
          `دورة تدريبية مهنية: ${category.title} بواسطة يوروكويست إنترناشيونال. برنامج مدرب من قبل الخبراء المصمم لتطوير المهارات والمهنية والتوظيف.`
        }
      ...
      <header>
        <HeroBanner
          ...
          title={category?.h1 || category?.title || "الفئات"}
      ...
      <main>
        <Container className="md:pb-12 pb-10">
          <section aria-label="بحث وعرض الكورسات">
